...
export default function WeatherWidget() {
  ...
  async function fetchWeather(cityName) {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Could not fetch weather. Try another city.')
      setWeather(null)
    } finally {
      setLoading(false)
    }
  ...
  return (
    <section className="section-padding bg-gradient-to-br from-slate-900 to-brand-900" ref={ref}>
      <div className="max-w-4xl mx-auto animate-on-scroll">
        <div className="text-center mb-10">
          <span className="text-accent text-sm font-semibold uppercase tracking-widest">Live API Integration</span>
        ...
        <form onSubmit={handleSearch} className="flex gap-3 max-w-md mx-auto mb-8">
          ...
          />
          <button type="submit" className="bg-brand-600 text-white px-5 py-3 rounded-xl hover:bg-brand-700 transition-colors">
            <Search size={18} />
          </button>
        </form>
          ...
          <div className="glass rounded-2xl p-8 text-white">
            <div className="flex flex-col md:flex-row items-center gap-8">
              ...
              <div className="text-center md:text-left">
                <div className="text-6xl font-display font-extrabold">{Math.round(weather.main.temp)}°C</div>
                <div className="text-white/60 capitalize mt-1">{weather.weather[0].description}</div>
                <div className="font-semibold text-xl mt-2">{weather.name}, {weather.sys.country}</div>
              </div>
              ...
              <div className="flex-1 grid grid-cols-2 gap-4 w-full">
                {[
                  ...
                  { icon: Cloud,      label: 'Visibility', value: `${(weather.visibility / 1000).toFixed(1)} km` },
                ].map(({ icon: Icon, label, value }) => (
                  <div key={label} className="bg-white/10 rounded-xl p-4 flex items-center gap-3">
                    <Icon size={18} className="text-brand-300 flex-shrink-0" />
                    <div>
